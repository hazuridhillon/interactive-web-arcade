...
interface Tile { id: string; type: number; x: number; y: number; matched: boolean; }
interface Match3GameProps { onClose: () => void; }
...
export const Match3Game = ({ onClose }: Match3GameProps) => {
  ...
  const createTile = useCallback((x: number, y: number, types: number): Tile => ({
    id: `${x}-${y}-${Date.now()}-${Math.random()}`, type: Math.floor(Math.random() * types), x, y, matched: false,
  }), []);

  const initializeGrid = useCallback(() => {
    const newGrid: Tile[][] = [];
    for (let y = 0; y < GRID_SIZE; y++) { newGrid[y] = []; for (let x = 0; x < GRID_SIZE; x++) newGrid[y][x] = createTile(x, y, tileTypes); }
    setGrid(newGrid);
  ...
  const checkMatches = useCallback((currentGrid: Tile[][]): boolean => {
    ...
    if (hasMatches) {
      ...
      setGrid(newGrid);
      setScore(prev => prev + matched.size * 10);
  ...
  const dropTiles = (currentGrid: Tile[][]) => {
    ...
    for (let x = 0; x < GRID_SIZE; x++) {
      ...
      for (let y = GRID_SIZE - 1; y >= 0; y--) {
        ...
        else if (empty > 0) { newGrid[y + empty][x] = { ...newGrid[y][x], y: y + empty, matched: false }; newGrid[y][x] = createTile(x, y, tileTypes); }
      }
      for (let y = 0; y < empty; y++) newGrid[y][x] = createTile(x, y, tileTypes);
    }
    setGrid(newGrid);
  ...
  const swapTiles = (t1: {x: number, y: number}, t2: {x: number, y: number}) => {
    ...
    setGrid(newGrid); setMoves(prev => prev - 1);
    setTimeout(() => {
      if (!checkMatches(newGrid)) {
        const revert = newGrid.map(row => row.map(tile => ({ ...tile })));
        const t = revert[t1.y][t1.x]; revert[t1.y][t1.x] = { ...revert[t2.y][t2.x], x: t1.x, y: t1.y }; revert[t2.y][t2.x] = { ...t, x: t2.x, y: t2.y };
        setGrid(revert); setMoves(prev => prev + 1);
      }
    }, 500);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden" style={{ backgroundColor: '#fff3e0' }}>
      <div className="relative rounded-[22px] p-6" style={{ backgroundColor: '#fff', boxShadow: '0 6px 0 0 #cc6e50' }}>
        <GameHeader
          title="Tile Match"
          badges={[
            { label: `Score: ${score}`, bg: '#fff176' },
            { label: `Moves: ${moves}`, bg: '#80e8ff' },
            ...(timeRemaining !== null ? [{ label: `${Math.floor(timeRemaining / 60)}:${(timeRemaining % 60).toString().padStart(2, '0')}`, bg: '#ff6fcf', color: '#fff' }] : []),
          ]}
          onClose={onClose}
        />

        {/* Grid */}
        <div className="grid gap-1 mx-auto rounded-[16px] p-2"
          ...
          {grid.map(row => row.map(tile => {
            ...
            return (
              <div key={tile.id} onMouseDown={() => handleMouseDown(tile.x, tile.y)} onMouseEnter={() => handleMouseEnter(tile.x, tile.y)}
                className="flex items-center justify-center cursor-grab active:cursor-grabbing transition-all duration-200"
                style={{
                  width: TILE_SIZE, height: TILE_SIZE, backgroundColor: style.bg, borderRadius: 10,
                  boxShadow: `0 3px 0 0 ${style.shadow}`, opacity: tile.matched ? 0 : 1,
        ...
        <p className="mt-3 text-center text-xs" style={{ fontFamily: "'Poppins', sans-serif", color: '#9b7abf' }}>Drag tiles to swap and match 3+</p>
      </div>
      ...
      {gameOver && (
        <GameOverModal
          ...
          onPlayAgain={() => { setScore(0); setGameOver(false); setDifficulty(null); }}
          onClose={onClose}
        />
      )}
    </div>
