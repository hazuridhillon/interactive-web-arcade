...
interface WordleGameProps { onClose: () => void; }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden y2k-lavender-bg">
      <div className="relative rounded-[22px] p-6 max-w-xl w-full mx-4" style={{ backgroundColor: '#fff', boxShadow: '0 6px 0 0 #d8b4fe' }}>
        <GameHeader title="Wordle" icon={<BookOpen className="w-6 h-6" style={{ color: '#b388ff' }} />} onClose={onClose} />
          ...
          {[...Array(MAX_ATTEMPTS)].map((_, rowIndex) => (
            <div key={rowIndex} className="flex gap-2 justify-center">
              {[...Array(WORD_LENGTH)].map((_, colIndex) => {
                ...
                return (
                  <div key={colIndex} className="w-14 h-14 flex items-center justify-center rounded-[10px] text-2xl transition-all duration-200"
                    style={{ fontFamily: "'Bungee', cursive", backgroundColor: s.bg, border: `3px solid ${s.border}`, color: s.color,
                      boxShadow: letter ? `0 3px 0 0 ${s.border}` : 'none', transform: letter ? 'scale(1.02)' : 'scale(1)' }}>
                    {letter}
                  </div>
                );
              })}
              ...
              {row.map(key => {
                ...
                return (
                  <button key={key} onClick={() => handleKeyClick(key)}
                    ...
                    style={{ fontFamily: "'Bungee', cursive", fontSize: key.length > 1 ? 9 : 12,
                      backgroundColor: s.bg, border: `2px solid ${s.border}`, color: s.color,
                      boxShadow: `0 3px 0 0 ${s.border}`, minWidth: key.length > 1 ? 52 : 32 }}>
                    {key === 'BACK' ? '←' : key}
                  </button>
                );
              })}
      ...
      {gameOver && (
        <GameOverModal
          title={won ? 'You Got It!' : 'Nice Try!'}
          titleColor={won ? '#69f0ae' : '#ff0098'}
          message={won ? `Solved in ${guesses.length} ${guesses.length === 1 ? 'try' : 'tries'}` : `The word was: ${targetWord}`}
